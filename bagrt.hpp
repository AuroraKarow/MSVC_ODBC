...
public:
    ...
    bool insert(_K &&key, _V &&value, bool move_assign = false)
    {
        ...
        {
            val[idx].occupy = true;
            val[idx].curr_mem = val.mem_size();
            ++ len;
            if(move_assign)
            {
                val[idx].key = std::move(key);
                val[idx].value = std::move(value); 
            }
            else
            {
                val[idx].key = key;
...
uint64_t string_hash(std::wstring src) { return string_hash(charset_exchange(src)); }
